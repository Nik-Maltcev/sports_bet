...
- ⏰ **Расписание**: Автоматическая отправка в 9:50 и 15:00 МСК
...
### 🔑 Настройка Perplexity AI (для реальных матчей)

**ВАЖНО**: Без API ключа бот использует моковые данные. Для получения реальных матчей:

1. Зарегистрируйтесь на [Perplexity AI](https://www.perplexity.ai/)
2. Получите API ключ в [настройках аккаунта](https://www.perplexity.ai/settings/api)
...
3. Добавьте новую переменную:
   ...
   - Value: ваш ключ API
4. Нажмите "Add" и перезапустите сервис

✅ **С API ключом**: Реальные матчи, актуальная статистика, профессиональный анализ
❌ **Без API ключа**: Моковые данные с реалистичными командами
...
## � Проверка работы Perplexity API
...
# Проверка в Railway (через логи)
curl https://your-app.railway.app/test -X POST
```

**Признаки работы API:**
- ✅ В прогнозах указаны реальные команды и турниры
- ✅ Упоминается актуальная статистика и трансферы  
- ✅ Анализ содержит конкретные факты о командах

**Признаки моковых данных:**
- ❌ Общие фразы без конкретной статистики
- ❌ Одинаковые шаблоны анализа
